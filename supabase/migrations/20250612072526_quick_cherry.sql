...
CREATE TABLE IF NOT EXISTS public.things_to_note (
...
ALTER TABLE public.things_to_note 
ADD CONSTRAINT fk_things_to_note_property_id 
FOREIGN KEY (property_id) REFERENCES public.properties(id) ON DELETE CASCADE;

-- Create index for better query performance
CREATE INDEX IF NOT EXISTS idx_things_to_note_property_id ON public.things_to_note(property_id);

-- Enable Row Level Security
ALTER TABLE public.things_to_note ENABLE ROW LEVEL SECURITY;

-- Create policy for public read access
CREATE POLICY "Allow public read access to things to note"
ON public.things_to_note
FOR SELECT
TO public
USING (true);
...
CREATE TRIGGER update_things_to_note_updated_at
    BEFORE UPDATE ON public.things_to_note
    FOR EACH ROW
